...
interface UpcomingGame {
  id: number; name: string; releaseDate: string | null; headerImage: string | null
  discountPercent: number; finalPrice: string | null
}
interface SteamNews {
          ...
          {(data?.upcomingGames ?? []).map((g, i) => (
            <a key={i} href={`https://store.steampowered.com/app/${g.id}`}
              ...
              <div className="min-w-0 flex-1">
                ...
                {g.releaseDate && <p className="text-[9px] text-slate-600 mt-0.5">📅 {g.releaseDate}</p>}
              </div>
              {g.finalPrice && (
                <span className="text-[11px] text-white shrink-0">€{g.finalPrice}</span>
              )}
            </a>
          ))}
        </div>
