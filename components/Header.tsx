...
export default function Header() {
  ...
  const [visitors, setVisitors] = useState<{ activeNow: number; todayPageViews: number; lifetimeViews: number } | null>(null)
  ...
  // Visitor tracking (moved from DaySummaryWidget)
  useEffect(() => {
    if (typeof window === 'undefined') return
    let sid = sessionStorage.getItem('infosk-sid')
    if (!sid) { sid = Math.random().toString(36).slice(2) + Date.now().toString(36); sessionStorage.setItem('infosk-sid', sid) }
    const doFetch = (action: string) => {
      fetch(`/api/visitors?action=${action}&sid=${sid}`)
        .then(r => r.json())
        .then(v => setVisitors(v))
        .catch(() => {})
    }
    doFetch('visit')
    const ping = setInterval(() => doFetch('ping'), 60000)
      ...
      <div className="max-w-[1680px] mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full px-3 py-1">
            ...
            {visitors && (
              <>
                <span className="text-[10px] font-bold text-emerald-300 font-mono tabular-nums">{visitors.activeNow}</span>
                <span className="hidden sm:inline text-[8px] text-slate-500">|</span>
                <span className="hidden sm:inline text-[9px] text-orange-300 font-mono tabular-nums">{visitors.todayPageViews}</span>
                <span className="hidden sm:inline text-[8px] text-slate-600">dnes</span>
                <span className="hidden lg:inline text-[8px] text-slate-500">|</span>
                <span className="hidden lg:inline text-[9px] text-slate-400 font-mono tabular-nums">{visitors.lifetimeViews.toLocaleString('sk-SK')}</span>
              </>
            )}
          </div>
        </div>
